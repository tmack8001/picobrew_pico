...
from . import main
from .config import brew_active_sessions_path, still_active_sessions_path, zseries_firmware_path
...
from .session_parser import active_brew_sessions, active_still_sessions
...
def create_session(token, body):
    ...
    folder = brew_active_sessions_path()
    
    # customize for still session (linking Z1 -> Still)
    if still_uid and body['SessionType'] == SessionType.STILL:
        if still_uid not in active_still_sessions:
            active_still_sessions[still_uid] = PicoStillSession()    
        
        active_session = active_still_sessions[still_uid]
        # associate the still session with the brewing device
        active_session.uid = uid   
        active_session.brewing_alias = '' if uid not in active_brew_sessions else active_brew_sessions[uid].alias
        folder = still_active_sessions_path()
        ...
        active_session.still_uid = None                 # clear still reference

    session_guid = uuid.uuid4().hex[:32]
    session_id = increment_session_id(uid, still_uid)
    ...
    if still_uid and active_session.type == SessionType.STILL:
        filename = '{0}#{1}-{2}#{3}#{4}#{5}.json'.format(
    ...
    active_session.filepath = folder.joinpath(filename)
    ...
    ret = {
        ...
        "Notes": None,
        "Pressure": body['PressurePa'],              # is this related to an attached picostill?
        "ProfileID": 28341,         # how to get the userId
        "SecondsRemaining": 0,
        "SessionLogs": [],
...
def update_session_log(token, body):
    ...
    active_session = active_brew_sessions[uid] if uid in active_brew_sessions else active_still_sessions[uid]
    ...
    if active_session.id == -1:
        ...
        active_session.id = session_id
        active_session.file = open(active_session.filepath, 'a')
...
def increment_session_id(uid, still_uid):
    if still_uid:
        return len(get_archived_sessions_by_machine(still_uid)) + (1 if active_still_sessions[still_uid].session != '' else 1)
    else:
...
def get_machine_by_session(session_id):
    brew_uid = next((uid for uid in active_brew_sessions if active_brew_sessions[uid].session == session_id or active_brew_sessions[uid].id == int(session_id) or active_brew_sessions[uid].id == -1), None)
    if brew_uid is None:
        return next((uid for uid in active_still_sessions if active_still_sessions[uid].session == session_id or active_still_sessions[uid].id == int(session_id) or active_still_sessions[uid].id == -1), None)
    return brew_uid
